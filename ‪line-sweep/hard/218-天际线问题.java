...
class Solution {
    public List<List<Integer>> getSkyline(int[][] buildings) {
        ...
        for (int boundary : boundaries) {
            ...
            for (int[] building : buildings) {
                if (building[0] <= boundary && building[1] > boundary && building[2] >= maxheight) {
                    maxheight = building[2];
                }
            }
        ...
        for (int i = 0; i < length; i++) {
            if (i == 0) {
                ans.add(list.get(0));
            } else if (ans.get(ans.size() - 1).get(1).intValue() != list.get(i).get(1).intValue()) {
                ans.add(list.get(i));
            }
        }
